...
```
Goals are now AI-powered: Recalculate and onboarding set your calories and macros from your profile, logged intake, and weight trend. AI uses your own provider key.
```
...
NEW
• Recalculate Goals now uses AI. It sets your daily calories and macros from your profile — and as you keep logging, refines them from your real intake and weight trend (a true "hit and trial" estimate of your maintenance). Uses your own provider key, and falls back to the standard formula when AI isn't available.
• Recalculate also refreshes your optional nutrient targets — fiber, sugar, sodium, and more.
• Onboarding now sets up your own provider, model, and key and builds your starting plan with AI.
...
No data migration is required. Existing logs, goals, widgets, and BYOK settings are preserved. Managed AI is not available for purchase or use.
```
...
AI ACCESS
Bring Your Own Key supports Gemini, OpenAI, Claude, Grok, Groq, OpenRouter, Together AI, Hugging Face, Fireworks AI, DeepInfra, Mistral, Ollama, or any OpenAI-compatible endpoint. Keys stay in iOS Keychain. AI features require a user-supplied provider key.
...
3) AI Access is Bring Your Own Key only. Users enter their own provider API key. Managed hosted AI cannot be selected, purchased, or used.
4) If a previous App Store subscription still appears on the Apple account, manage, cancel, or request a refund in Settings → Apple Account → Subscriptions. The app does not sell or fulfill managed AI access in this version.
5) BYOK review path: go to Settings → AI Access → Bring Your Own Key → enter any valid Gemini API key. A free key can be obtained at https://aistudio.google.com/apikey
```
