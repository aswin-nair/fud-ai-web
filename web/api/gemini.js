...
const FALLBACK_MODELS = {
  ...
  coach: ["gemini-3.1-flash-lite", "gemini-2.5-flash-lite", "gemini-2.5-flash"],
};
...
function quotaKey(installID, task) {
  const base = `fudai:premium:${todayKey()}:${installID}`;
  return task ? `${base}:${task}` : base;
}

function taskDailyLimit(task) {
  const envKey = {
    food: "FUD_AI_PREMIUM_FOOD_DAILY_LIMIT",
    speech: "FUD_AI_PREMIUM_SPEECH_DAILY_LIMIT",
    coach: "FUD_AI_PREMIUM_COACH_DAILY_LIMIT",
  }[task];
  return positiveInteger(process.env[envKey], DEFAULT_TASK_LIMITS[task]);
}

function globalDailyLimit() {
  return positiveInteger(process.env.FUD_AI_PREMIUM_GLOBAL_DAILY_LIMIT, DEFAULT_GLOBAL_LIMIT);
}
