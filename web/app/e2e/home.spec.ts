...
test.describe('Home & food logging', () => {
  ...
  test('keeps meals visible and opens optional daily extras on request', async ({ page }) => {
    ...
    await page.locator('.poiem-daily-extras > summary').press('Enter')
    ...
    await page.getByRole('button', { name: 'Glass 2', exact: true }).click()
    await page.locator('.poiem-daily-extras > summary').press('Enter')
    await expect(page.getByRole('group', { name: 'Water glasses' })).toBeHidden()
    await expect(page.locator('.poiem-daily-extras > summary')).toContainText('2/8 water')
    await page.reload()
    await expect(page.locator('.poiem-daily-extras > summary')).toContainText('2/8 water')
