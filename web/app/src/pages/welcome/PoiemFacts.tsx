...
/** The principles, set as a nutrition label for an ordinary day. */
  ...
  return (
    <section className="wp-section" id="principles" aria-labelledby="principles-title">
      <div className="wp-wrap wp-facts-grid">
        ...
        <figure className="wp-facts wp-nl" aria-labelledby="facts-title">
          ...
          <dl className="wp-facts-rows">
            {ROWS.map(row => (
              <div key={row.label}><dt>{row.label}</dt><dd className="tabular">{row.value}</dd></div>
            ))}
          </dl>
          <span className="wp-nl-rule is-thick" aria-hidden="true" />
