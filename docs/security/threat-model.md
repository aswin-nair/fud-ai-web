...
2. Browser storage: holds app state, local account records, a session descriptor, BYOK settings, notification state, and local analytics. Access tokens stay in memory.
...
5. Neon boundary: stores users, an opaque JSON snapshot, and additive entity/migration tables. The snapshot remains authoritative.
...
8. Expo device boundary: SQLite contains profile, meals, food records, gamification ledgers, and a secret-free sync outbox. Refresh credentials and the device id live in SecureStore; access tokens stay in memory.
...
| TM-006 | Session theft/replay or account takeover | Short-lived `use: access` JWT in memory; rotating hashed refresh cookie on web; mobile refresh JSON only when `ENABLE_MOBILE_AUTH` is on and no browser Origin is present; family replay revocation; logout, logout-all, password change, and deletion revoke sessions; scrypt cloud passwords; HMAC rate-limit keys; provider-collision refusal | Local development auth still stores password verifiers in browser storage; hosted cookie/replay and mobile SecureStore evidence are still required | High | Staging expiry/revocation, refresh-replay, cross-tab, and native SecureStore tests; external review |
| TM-007 | Duplicate/replayed meal mutation awards duplicate XP or corrupts streak | Web keeps an untruncated award-key ledger; entity mutations hash the full envelope and replay the same cursor; derived streak ignores client XP | Live clients still write snapshots; hosted entity dual-write and XP extraction are still required | High | Concurrent/replay integration tests and ledger reconciliation |
...
| TM-009 | Sensitive values enter analytics, crash reports, or server logs | Analytics and API logs use a typed versioned allowlist and rejection tests; ordinary state paths do not log payloads | Apply the same fixtures to a reviewed remote sink before enabling collection | Critical | Repo-side redaction fixtures plus future hosted sink evidence |
