...
| TM-006 | Session theft/replay or account takeover | Signed minimal-claim JWT bound to a database session; current/all-session revocation; scrypt cloud passwords; database token-bucket limits with HMAC keys; provider-collision refusal | Access token remains a 30-day bearer in browser localStorage and there is not yet a short-access/rotating-HttpOnly-refresh design; local development auth stores password verifiers in browser storage | High | Staging expiry/revocation and cross-tab tests; external review; migrate to short-lived access plus rotating HttpOnly session/refresh design |
