...
|---|---|---|---|---|---|
| TM-001 | BYOK key reaches Neon through whole-state sync | Client secret-free serializer, separate device-local key, server-side stripping, and purge on load are present in the hardening path | Add strict contract tests, inspect and bulk-purge historical/dormant rows if applicable, and prevent future DTOs from representing the key | Critical | Serialization and API rejection tests using canary secret values; storage inspection and bulk-purge procedure |
...
| TM-003 | Malformed or oversized `PUT /api/state` corrupts state or consumes storage/compute | API requires a bearer token and the hardening path adds a top-level shape check and 2 MB limit | Add a strict field-level runtime schema, per-collection/string limits, unknown-key policy, and structured errors | High | Boundary, fuzz, and payload-limit integration tests |
| TM-004 | A failed cloud read hydrates fresh state and a later autosave overwrites valid remote history | Client catches load errors | Separate `unavailable`, `not found`, and `loaded`; never arm writes after a failed read; use versions/preconditions | Critical | Forced timeout/500 tests proving zero remote writes and retained local data |
...
| TM-006 | Session theft/replay or account takeover | Signed HS256 JWT; server hashes cloud passwords with scrypt | Current JWT lasts 30 days and has no rotation/revocation; auth endpoints lack rate limits; local auth stores password verifiers in browser storage | High | Rotation/revocation tests, brute-force/rate-limit tests, session inventory and logout-all test |
| TM-007 | Duplicate/replayed meal mutation awards duplicate XP or corrupts streak | Current web XP events have dedup keys; mobile points ledger is append-only | Future API needs client mutation ID, idempotency key, server validation, transaction boundaries, and replay tests | High | Concurrent/replay integration tests and ledger reconciliation |
...
| TM-009 | Sensitive values enter analytics, crash reports, or server logs | Current analytics is a local typed ring buffer | Introduce a versioned allowlist; prohibit birthday, body values, food text, photos, chat, tokens, and keys; add redaction at every sink | Critical | Canary-secret and sensitive-field rejection tests for client, API, logs, and crash sink |
| TM-010 | Export unintentionally includes secrets/private chat, or import overwrites good data | User explicitly initiates JSON export/import; the hardening path strips the BYOK key | Disclose included private categories, validate/version imports, preview changes, and retain rollback; keep an allowlist test so later fields do not leak | High | Export field allowlist test, malicious import suite, round-trip and rollback tests |
| TM-011 | Delete UI resets visible state but leaves account, auxiliary browser stores, backups, or server data | Local state key can be cleared; Neon state can be replaced with fresh JSON | Specify clear-log versus delete-account semantics; enumerate stores and retention; implement authenticated deletion status and completion evidence | High | End-to-end deletion test with database, storage, backup/retention, and UI assertions |
...
| TM-014 | Privacy or security disclosure contradicts deployed accounts/cloud behavior | Deployment guide documents Neon mode | Current public and security documents describe web as static/no-backend and say no accounts/cloud sync | High | Release checklist comparing deployed data flows to privacy, security, terms, and in-product copy |
